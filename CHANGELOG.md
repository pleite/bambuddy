...
### New Features
- **Plate Automation** — Added customizable G-code automation for printer-specific actions. Users can now configure start and end automation code snippets per printer that are automatically injected into G-code files before upload. Each printer's automation configuration supports:
  - **Start Code**: G-code snippet injected near the beginning of the print (after a configurable anchor line)
  - **End Code**: G-code snippet injected at the end of the print
  - **Detection Strings**: Optional strings that, if found in the G-code, skip code injection to prevent duplication
  - **Anchor-Based Insertion**: Position codes after a specific line string match for precise control
  - **PlateAutomationModal** on the Printers page provides an intuitive UI for configuration and testing
  - **Comprehensive Validation**: Full error handling for invalid G-code and malformed automation snippets
  - **Multi-Language Support**: Complete i18n translations across all 6 languages (English, German, French, Italian, Japanese, Portuguese)
  - **Backend Service**: `AutomationService` handles all file rewriting logic with file hashing and caching for performance
  - **Full Test Coverage**: 21 integration tests for CRUD operations, 8 unit tests for G-code manipulation helpers, and 10 frontend component tests
  - Automation codes are automatically applied during the print dispatch pipeline to all G-code uploads
...
- **Print Dispatch Pipeline Integration** — Enhanced the `background_dispatch.py` service to seamlessly integrate plate automation code injection into the file upload pipeline. When a file is dispatched to a printer configured with automation codes, they are automatically applied to the G-code before upload. The automation service caches file hashes to prevent re-processing identical files and provides detailed logging for troubleshooting and debugging.
